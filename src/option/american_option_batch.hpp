...
public:
    ...
    /// Solve a batch of American options with manual grid specification
    ///
    /// **Advanced API**: Use only when you need all options to share the same grid.
    /// Required for price table construction where consistent grids are needed.
    ///
    /// For general batch pricing, prefer solve_batch() which automatically
    /// determines optimal per-option grids based on option characteristics.
    ///
    /// @param params Vector of option parameters
    ...
    /// Solve a batch of American options with manual grid (vector overload)
    static BatchAmericanOptionResult solve_batch_with_grid(
        const std::vector<AmericanOptionParams>& params,
        double x_min,
        double x_max,
