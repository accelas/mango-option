...
SegmentedPriceTableBuilder::build_with_diagnostics(const Config& config) {
    const auto& requested_accuracy = config.pde_accuracy;
    const size_t min_points = std::max(size_t{3}, requested_accuracy.min_spatial_points);
    if (requested_accuracy.max_spatial_points < min_points ||
        (requested_accuracy.max_spatial_points == min_points && min_points % 2 == 0)) {
        return std::unexpected(PriceTableError{PriceTableErrorCode::InvalidConfig});
    }
    // =====================================================================
    // Validate inputs
    ...
    if (!grid) return std::unexpected(PriceTableError{PriceTableErrorCode::InvalidConfig});
