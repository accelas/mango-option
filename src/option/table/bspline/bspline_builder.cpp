    ...
    const PriceTableAxesND<N>& axes) const
{
    ...
    auto estimate = estimate_batch_pde_grid(std::span<const PricingParams>(batch), accuracy);
    ...
    const PriceTableAxesND<N>& axes) const
{
    ...
    return std::visit([&](auto&& grid) -> BatchAmericanOptionResult {
        ...
        if constexpr (std::is_same_v<T, GridAccuracyParams>) {
            ...
            auto estimate = estimate_pde_grid(batch, axes);
            if (!estimate) {
                BatchAmericanOptionResult failure;
                failure.failed_count = batch.size();
                failure.results.reserve(batch.size());
