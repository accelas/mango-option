    ...
    std::span<const double> rate_nodes)
{
    ...
    std::fprintf(stderr,
        "[DEEP] solve_missing_pde_pairs: tau_vec.size=%zu first=%.6f last=%.6f "
        "batch.size=%zu batch[0].divs=%zu batch[0].maturity=%.6f\n",
        tau_vec.size(),
        tau_vec.empty() ? 0.0 : tau_vec.front(),
        tau_vec.empty() ? 0.0 : tau_vec.back(),
        batch.size(),
        batch.empty() ? 0u : batch[0].discrete_dividends.size(),
        batch.empty() ? 0.0 : batch[0].maturity);

    auto batch_result = solver.solve_batch(
        std::span<const PricingParams>(batch), /*use_shared_grid=*/true);

    std::fprintf(stderr,
        "[DEEP] batch_result: results.size=%zu failed=%zu\n",
    ...
    for (size_t bi = 0; bi < missing.size(); ++bi) {
        ...
        if (!batch_result.results[bi].has_value()) {
            std::fprintf(stderr, "[DEEP] result[%zu] has no value\n", bi);
            continue;
        }
        ...
        if (bi == 0) {
            ...
            std::fprintf(stderr,
                "[DEEP] result[0]: x_grid.size=%zu num_snapshots=%zu has_snapshots=%d\n",
                x_grid.size(), result.num_snapshots(),
                static_cast<int>(result.has_snapshots()));
            std::fprintf(stderr, "[DEEP] snap_times[0..5]: ");
            ...
            for (size_t j = 0; j < tau_nodes.size() && j < 4; ++j) {
                ...
                if (!spatial.empty()) {
                    minv = maxv = spatial[0];
                    for (double v : spatial) {
                        minv = std::min(minv, v); maxv = std::max(maxv, v); sumv += v;
                    }
                }
                std::fprintf(stderr,
                    "[DEEP] result[0].at_time(%zu): n=%zu min=%.6g max=%.6g sum=%.6g first=%.6g\n",
...
ChebyshevSegmentedBuilder::build_adaptive(
