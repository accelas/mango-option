...
for the 1D fitter, 1e-6 per axis for separable fitting). Interpolation error
between nodes is checked separately in numerical regression tests. Strongly
nonuniform sites can remain ill-conditioned; the condition estimate remains
part of the diagnostics. Builder tests validate final prices and Greeks.
