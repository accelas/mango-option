...
Chebyshev-Gauss-Lobatto node generation preserves the supplied endpoints exactly
for every caller, including continuous and segmented tables. Interior nodes keep
the same cosine formula. Endpoint rounding can therefore differ by a few ULPs
from older builds; this prevents interval ownership errors during sampling.
