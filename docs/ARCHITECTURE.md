...
**Certified construction and root conditioning.** Public factories, manual publication and loading prove the final represented physical price nondecreasing in sigma after its coordinate transforms, floors, European add-back, temporal routing and reference-strike blend. The proof is bound to an immutable payload and its actual admitted domain; loading recomputes it. A witnessed violation and an indeterminate proof have distinct typed failures. Interpolated IV accepts only those certified library tables. Every successful root then passes a finite positive-vega check, the configured sensitivity floor, and a representable-price-resolution check. Valid flat prices remain priceable but can have unidentifiable IV. This mandatory behavior replaces the optional 17-point multiple-root screen and its configuration toggle; it does not itself establish approximation accuracy.
