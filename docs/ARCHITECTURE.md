...
|------|---------|----------|
| **FDM pricing** | ~0.3ms ATM, ~6ms OTM, ~19ms deep ITM | Single option or small batch |
...
The interpolated solver replaces the nested PDE solve with a lookup into a pre-computed price surface. Brent's method on the smooth surface converges in ~4-6 function evaluations, each requiring only a surface evaluation (~200ns). Total IV solve: ~3.5us — a ~2,000x speedup over FDM.
