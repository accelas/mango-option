...
3. **Item 6, `CubicSpline2D::eval` extrapolation** — options: (a) clamp the
   ...
   nearest-boundary is the safe behavior for pricing surfaces. Zero risk:
   `CubicSpline2D` has no production users (only `tests/cubic_spline_2d_test.cc`).
4. **Item 7, factory error collapsing** — options: (a) one new catch-all code;
   ...
   restructure to propagate the underlying code. **Chosen: (a).** Add
   `ValidationErrorCode::SurfaceBuildFailed`; stops the `InvalidGridSize` lie
   with minimal enum growth; per-code granularity deferred until someone needs
   it.
...
the actual step is shrunk so the grid lands exactly on `t_end`. No validation
added (parity with the current constructor, which accepts any inputs).

**Test:** `TimeDomain(0.0, 1.0, 0.3)` → `n_steps() == 4`, `dt() == 0.25`,
`time_points().back() == 1.0` (exact); exact-multiple case `(0, 1, 0.25)`
unchanged.
...
`solve()` rather than `initialize()` makes each solve self-contained
regardless of whether the caller re-initializes.

**Test:** solve the same instance twice with a temporal event that visibly
changes the solution; assert the second run's result equals the first run's
(today the second run differs because the event is skipped).
...
- `src/math/bspline/bspline_nd.hpp:271` — `coeffs_view_` over `coeffs_`.
  **Fix:** delete copy constructor/assignment; explicitly default move
  ...
  target). All in-repo uses are `shared_ptr<const BSplineND>` built by move
...
- `src/pde/core/grid.hpp:547` — `NonUniformSpacing::sections_view_` over
  ...
  constructor/assignment (same vector-move argument). Keeps
  `Grid`/`SpacingVariant` copyable.

**Tests:** for `NonUniformSpacing`: copy an instance, let the source be
destroyed (scope), assert `copy.sections_view_.data_handle() ==
copy.precomputed.data()` and values still read correctly. Same
`data_handle()`-identity assertion after a move. For `BSplineND`:
...
the success path is dead code. The PSOR/obstacle variant duplicates the bug
(`:336` guard vs `:356` trivial case).
...
**Fix:**
- Add `ValidationErrorCode::SurfaceBuildFailed` (appended at the end of the
  enum) with a message in the `validation_error_message` switch
  (`error_types.hpp` ~`:265`).
- Map the `default:` arm and the five hardcoded sites to `SurfaceBuildFailed`
  (payload `value`/`index` preserved where available).
- Move `to_validation_error` to `namespace mango::detail` and declare it in
  the factory's header so it is directly unit-testable.
...
still-collapsed `PriceTableErrorCode` maps to `SurfaceBuildFailed`, and the
existing specific arms are unchanged.
...
**Fix (docs only, zero behavior change):** re-document `tolerance` as the
absolute residual tolerance |f(x)|; re-document `brent_tol_abs` as an
absolute tolerance applied to both the residual and the bracket width, noting
...
**Fix:** `static_assert(Bandwidth == 4, ...)` in the class body (covers the
workspace's `KL/KU = Bandwidth − 1` underflow for 0 as well, since the
workspace is only reachable through the collocation class). The parameter
stays for future generality.

**Test:** compile-time; no runtime test. (The static_assert *is* the
regression guard.)
...
provides `lapack_int` there.

**Test:** `static_assert(std::same_as<decltype(ws.pivots())::element_type,
lapack_int>)` in the workspace test; behavior under LP64 unchanged (existing
tests cover).
...
**Test:** force a convergence failure (tiny `max_iter` + tight tolerance on a
problem needing more iterations); assert the reported `residual > 0`.
...
- Per-code error taxonomy beyond `SurfaceBuildFailed` (rejected in
  brainstorm).
