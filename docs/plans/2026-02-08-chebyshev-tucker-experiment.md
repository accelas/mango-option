...
Throughout this document, `num_pts` is the number of sample points per axis.
For Chebyshev, this means polynomial degree n = num_pts - 1.

### Chebyshev Interpolation

Chebyshev-Gauss-Lobatto nodes on [-1, 1]:

    xⱼ = cos(jπ / n),  j = 0, ..., n    (n+1 = num_pts nodes)
...
**Four interpolant configurations:**
1. **B-spline (cubic, unsegmented)** — Single surface over full domain
2. **B-spline (cubic, 3 segments)** — Segmented ln κ axis, matching PR 386
3. **Chebyshev full tensor** — No Tucker compression, isolates basis effect
4. **Chebyshev-Tucker** — HOSVD compression with log-spaced threshold sweep

Configurations 1-2 establish the B-spline accuracy ceiling (unsegmented hits
the wall; segmented is the best known workaround). Configuration 3 isolates
the Chebyshev basis effect from Tucker compression. If Chebyshev-full breaks
the wall but Chebyshev-Tucker doesn't, the issue is rank truncation.
