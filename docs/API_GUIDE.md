...
at fixed moneyness, linearly in inverse strike. Reference strikes should
span and resolve the query range.
...
Reference strikes should cover the strike range implied by the moneyness grid:
`S/K ∈ [0.92, 1.08]` at spot 100 corresponds to strikes in `[92.6, 108.7]`.
The blend interpolates normalized cash amounts at fixed moneyness. Manual
construction has no viability gate, so validate the resulting accuracy.
...
Adaptive grid also works with discrete dividends. The builder probes 2–3 representative K_ref values using segmented PDE surfaces, merges the knot positions each probe's refinement chose (union per axis, thinned to `max_points_per_dim`), then builds all segments on those merged grids:
...
The reference surfaces preserve query moneyness and interpolate normalized
prices in inverse strike. For bracketing references `L` and `H`,
`w = H/K * (K-L)/(H-L)` and the price is
`K * [(1-w) * P(S*L/K,L)/L + w * P(S*H/K,H)/H]`.
This interpolates the normalized cash amounts `D/K`; it preserves the
no-cash homogeneous limit. Delta and Gamma include the spot-remapping chain
rule. Outside the reference span the nearest normalized-dividend problem
is used, so accuracy still needs validation.

Adaptive B-spline builds seed maturity knots in square-root elapsed time
within every dividend interval, starting at the exact expiry payoff. They
retain physical times through refinement, merging, and final assembly.
The volatility seed is filled to seven sites when the point cap permits, to
resolve nearly flat exercise regions before random refinement probes. Each event closes two separate fitted segments. The builder adds
only the endpoints and cubic support needed by each segment. Raw PDE samples
use at least 201 spatial points, increasing with the fitted moneyness count.
This raises construction cost to reduce the source-price accuracy floor.

`BSplineSegmentedAdaptiveResult::tau_grid` reports the retained global knots;
`tau_points_per_segment` reports the largest actual fitted segment size.
For direct manual builds, `SegmentedPriceTableBuilder::Config::tau_grid`
accepts explicit sorted finite times in `[0,T]`. An empty vector retains the
count-based manual sampling policy. Saved tables retain their sampled knots;
rebuild them to benefit from improved maturity sampling.
...
auto estimate = mango::estimate_pde_grid(params, accuracy);
if (!estimate) {
...
existing typed errors. `estimate_pde_grid`, `estimate_batch_pde_grid`, and
`estimate_batch_pde_grid_config` return `std::expected` results with
`ValidationError`; callers must check success before using the grid.
`validate_grid_accuracy` provides the same typed preflight check.
...
Coverage widening does not reinterpret or retune it. Concentrations must be
positive, finite, and at most `log(numeric_limits<T>::max())`; larger requests
return `InvalidGridSpacing`. Subnormal concentrations use the linear limit.
Normalized reuse preserves the profile's `n_sigma` clearance at every original
member, even when members are far from the normalized center. The default three-diffusion-
...
For a whole batch, `estimate_batch_pde_grid_config(batch, acc)` returns
`std::expected<PDEGridConfig, ValidationError>`. After checking success, pass
its value as `solve_batch`'s `custom_grid`.
...
| `surface_failures` | Holdout points where the round trip on the returned surface failed: no root, ambiguous, non-convergent, non-finite, or vega below the pre-check threshold. |
