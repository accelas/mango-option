...
$$u_{n+1} = \frac{1}{\gamma(2-\gamma)}u_\text{stage} - \frac{(1-\gamma)^2}{\gamma(2-\gamma)}u_n + \frac{(1-\gamma)\Delta t}{2-\gamma}\mathcal{L}(u_{n+1})$$
...
**Drift discretization (issue #472).** One-pass exactness also assumes the stage Jacobian is an M-matrix (non-positive off-diagonals). Here $a = \sigma^2/2$ and $b = r(t) - q - \sigma^2/2$ denote the diffusion and drift coefficients of $\mathcal{L}$, not the tridiagonal bands $a_i, b_i, c_i$ above. The drift term uses an Il'in exponentially-fitted diffusion coefficient rather than the plain centered stencil: per interior cell,

$$a_f = a \cdot \rho \coth(\rho), \qquad \rho = \frac{|b| \, h_{\text{binding}}}{2a},$$

where $h_{\text{binding}}$ is the neighbor spacing on the side the drift can flip — $dx_{\text{right}}$ for $b > 0$, $dx_{\text{left}}$ for $b < 0$. Since $\rho \coth(\rho) \geq \max(1, \rho)$, the fitted off-diagonals of $\mathcal{L}$ are non-negative for every $\sigma$/$h$/$b$ combination — the cell-Péclet failure mode of the former centered drift stencil (an off-diagonal entry could flip sign at low volatility on a coarse grid, e.g. σ = 1%, h = 0.1) is removed unconditionally, at the cost of $O(\rho^2)$ added numerical diffusion (the scheme stays second-order consistent on refining grids and degrades gracefully to upwind-like behavior in the convection-dominated limit). The fitted coefficient is cached per sampled $(a, b)$ and grid, with the cache's validity metadata stored alongside it in the solver workspace, so re-fitting on every apply/assemble call — which would otherwise put a `tanh` on the hot path — is avoided; the cache only recomputes when the sampled drift actually changes between stages.
...
One-pass Brennan-Schwartz exactness still requires, beyond these off-diagonal signs: (1) strict row dominance $1 + \omega \cdot r(t) > 0$, where $\omega$ is the implicit stage weight (i.e. $\gamma\Delta t/2$ or $(1-\gamma)\Delta t/(2-\gamma)$) — automatic for $r \geq 0$ and violated only at absurd negative rates $r \leq -1/\omega \approx -2/\Delta t$, which the API accepts and therefore remains a documented caveat (the solver does validate the TR-BDF2 $\gamma \in (0, 1)$, so stage weights are always positive); and (2) an interval active set touching the sweep's starting side (issue #473). `validate_lcp_kkt` checks the solved system's KKT conditions — it detects resulting solution defects, not matrix structure, so a clean report is expected but not a structural guarantee.

The ghost-eliminated boundary rows deliberately retain the raw (unfitted) diffusion coefficient $a$: their off-diagonal $+2a/h^2$ already has the required sign for any drift (drift enters those rows only through the affine term), so fitting them would add diffusion without buying any structural property. The deep-ITM exercise lock in `solve_implicit_stage_projected` also deliberately evaluates $L(\psi)$ with the raw (unfitted) operator (`SpatialOperator::apply_unfitted`) rather than the fitted one used for residuals/RHS/Jacobian: that lock criterion is a physical question — is the payoff a strict subsolution of the continuous PDE? — and the fitted diffusion's extra $(a_f - a)\cdot\psi''$ biases the answer negative for puts ($\psi'' < 0$ deep ITM), which can over-lock continuation-valued nodes to intrinsic on coarse, asymmetric grids.
...
The solution: pre-compute prices across a 4D parameter grid (moneyness, maturity, volatility, rate), fit a B-spline surface, and evaluate the surface at ~250ns per query. This section covers the interpolation machinery, grid estimation, and IV extraction.
...
Each derivative costs the same as a price evaluation (~250ns) because we evaluate one differentiated 1D basis and three undifferentiated ones.
...
The key advantage: both $P$ and $\partial P / \partial\sigma$ come from B-spline evaluation (~250ns each), not PDE solves. Newton's method converges quadratically (error squares each iteration), typically in 3–4 iterations.
...
The interpolated solver is ~2,000× faster, at the cost of pre-computation time and interpolation error (typically <5 bps IV error in the core region with adaptive grid refinement).
