...
// Regression: manual levels must be representable and affordable.
// Bug: Checking only size_t overflow admitted multi-gigabyte node allocations.
TEST(ChebyshevPDECacheTest, RejectsUnrepresentableManualLevels) {
    ...
    invalid_levels.push_back({31, 1, 1, 1});  // representable but impractical allocation
