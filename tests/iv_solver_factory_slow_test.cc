...
// Regression: continuous Chebyshev must honor adaptive configuration.
// Bug: Factory dispatch silently discarded adaptive controls.
...
// Regression: short-expiry adaptive tables must refuse later maturities.
// Bug: The minimum half-year sample span widened past the requested expiry.
TEST(IVSolverFactoryChebyshev, ContinuousAdaptiveCapsShortMaturity) {
    for (double maturity : {0.005, 0.05, 0.25, 0.5}) {
        SCOPED_TRACE(maturity);
        auto config = make_base_config();
        config.option_type = OptionType::CALL;
        config.dividend_yield = 0.0;
        // Keep the very short-expiry holdout near ATM so enough samples
        // have measurable IV; the test concerns maturity admission.
        config.grid.moneyness = {0.99, 1.0, 1.01};
        config.grid.vol = {0.15, 0.25, 0.35};
        config.adaptive = AdaptiveGridParams{
        ...
        PricingParams p(OptionSpec{
            ...
            .rate = 0.05, .option_type = OptionType::CALL}, 0.25);
        EXPECT_TRUE(table->validate_pricing_params(p));
        p.maturity = std::nextafter(std::min(0.01, maturity * 0.5), 0.0);
        EXPECT_FALSE(table->validate_pricing_params(p));
        p.maturity = std::nextafter(maturity, 1.0);
        EXPECT_FALSE(table->validate_pricing_params(p));
        auto solver = table->make_iv_solver();
        ASSERT_TRUE(solver);
        auto result = solver->solve(IVQuery(static_cast<const OptionSpec&>(p), 1.0));
        ASSERT_FALSE(result);
        EXPECT_EQ(result.error().code, IVErrorCode::InvalidGridConfig);
    }
}
