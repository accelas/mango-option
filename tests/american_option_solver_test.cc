...
TEST(AmericanOptionSolverTest, InvalidGridNTime) {
    ...
    // Note: PDEWorkspace doesn't take n_time, temporal validation is done by solver
    // This test now verifies that grid creation succeeds
    auto grid_spec = GridSpec<double>::uniform(-3.0, 3.0, 101).value();
    auto result = PDEWorkspace::create(grid_spec, std::pmr::get_default_resource());
