...
#include "src/option/price_table_snapshot_collector.hpp"  // For SnapshotInterpolator
...
TEST(SnapshotInterpolatorTest, DerivedSplineInvalidatedOnGridChange) {
    ...
    // Capture epoch after first build
    uint64_t epoch1 = interp.current_epoch();

    // Evaluate derivative (this caches derived spline)
    double d1 = interp.eval_from_data(1.5, std::span{deriv1}, epoch1);
    ...
    // Capture epoch after second build (should be same as before since build doesn't increment)
    uint64_t epoch2 = interp.current_epoch();
    ...
    double d2 = interp.eval_from_data(0.75, std::span{deriv2}, epoch2);
...
TEST(SnapshotInterpolatorTest, RebuildSameGridPreservesDerivedCache) {
    ...
    // Capture new epoch after rebuild (should be incremented)
    uint64_t epoch2 = interp.current_epoch();
    EXPECT_GT(epoch2, epoch1);  // Epoch should increment on rebuild_same_grid

    // Evaluate derivative (should use cached grid structure)
    double d2 = interp.eval_from_data(1.5, std::span{deriv2}, epoch2);
...
TEST(SnapshotInterpolatorTest, DerivedSplineFallbackOnError) {
    ...
    // Capture epoch after build
    uint64_t epoch = interp.current_epoch();

    // eval_from_data with empty data should fallback to linear (return 0)
    std::vector<double> empty_data;
    double result = interp.eval_from_data(0.5, std::span{empty_data}, epoch);
    EXPECT_EQ(result, 0.0);

    // eval_from_data with valid data should work
    std::vector<double> valid_data = {1.0, 2.0, 3.0};
    result = interp.eval_from_data(0.5, std::span{valid_data}, epoch);
