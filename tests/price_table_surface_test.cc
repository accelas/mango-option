...
// This test ensures the return type is shared_ptr<const PriceTableSurfaceND<N>>, not
// shared_ptr<const PriceTableSurfaceND> (which would be invalid)
TEST(PriceTableSurfaceTest, BuildReturnsCorrectTemplateType) {
    ...
    // This would fail to compile if build() returned shared_ptr<const PriceTableSurfaceND>
    // without the <N> template parameter
    std::shared_ptr<const PriceTableSurfaceND<2>> surface = result.value();
...
TEST(PriceTableSurfaceTest, Build3DReturnsCorrectTemplateType) {
    ...
    std::shared_ptr<const PriceTableSurfaceND<3>> surface = result.value();
    EXPECT_NE(surface, nullptr);
}
