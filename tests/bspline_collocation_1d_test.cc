...
TEST_F(BSplineCollocation1DTest, ExtremelyClustered) {
    ...
    if (result.has_value()) {
        auto spline = BSplineND<double, 1>::create(
            {grid}, {clamped_knots_cubic(grid)}, std::move(result->coefficients));
        ASSERT_TRUE(spline);
        // Knot placement changes the collocation condition number. Check
        // the interpolation equations against the independent linear data;
        // no fixed condition-number floor is a correctness requirement.
        EXPECT_TRUE(std::isfinite(result->condition_estimate));
        for (double x : grid) EXPECT_NEAR(spline->eval({x}), x, 1e-8);
    } else {
        EXPECT_EQ(result.error().code, mango::InterpolationErrorCode::FittingFailed);
    }
}
