...
static void BM_InterpolationGreekAccuracy(benchmark::State& state) {
    ...
    auto solve_price = [&](double K, double tau) -> double {
        PricingParams p{OptionSpec{.spot = spot, .strike = K, .maturity = tau, .rate = rate, .dividend_yield = fix.dividend_yield, .type = OptionType::PUT}, sigma};
    ...
    for (double K : strikes) {
        for (double tau : maturities) {
            PricingParams params{OptionSpec{.spot = spot, .strike = K, .maturity = tau, .rate = rate, .dividend_yield = fix.dividend_yield, .type = OptionType::PUT}, sigma};
