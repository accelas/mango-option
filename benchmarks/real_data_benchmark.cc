...
const AnalyticSurfaceFixture& GetAnalyticSurfaceFixture() {
    static AnalyticSurfaceFixture* fixture = [] {
        ...
        fixture_ptr->tau_grid = {0.01, 0.05, 0.1, 0.25, 0.5};  // Short maturities like real data
        fixture_ptr->sigma_grid = {0.10, 0.15, 0.20, 0.25, 0.30};
        fixture_ptr->rate_grid = {0.0, 0.02, 0.04, 0.06};
...
static void BM_RealData_GridEstimator(benchmark::State& state) {
    ...
    auto builder_result = PriceTableBuilder<4>::from_chain_auto(
        iv_fixture.chain,
        GridSpec<double>::uniform(-3.0, 3.0, 101).value(),
        200,
        OptionType::PUT,
        accuracy
    );
