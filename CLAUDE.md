...
Adaptive builds validate with the operational round trip: each reference
price is inverted on the candidate with the shipped solver's own inversion,
so a build returns `NoViableSurface` when a resolved holdout sample cannot
